...
// Function to dynamically generate a 24-hour baseline vs actual trend
const generateCommuteChartData = (baselineSpeed: number, currentSpeed: number) => {
  const data = [];
  const currentHour = 17; // Mock current time as 17:00 (5 PM)
  
  for (let i = 0; i <= 23; i++) {
    // Create a mock curve for baseline speed (drops during rush hours 8am, 5pm)
    let normal = baselineSpeed;
    if (i >= 7 && i <= 9) normal -= 15; // Morning rush
    if (i >= 16 && i <= 19) normal -= 20; // Evening rush
    ...
    let actual = normal;
    if (i === currentHour) {
      actual = currentSpeed; // Big drop right now
    } else if (i === currentHour - 1) {
      actual = normal - (normal - currentSpeed) * 0.4; // Started dropping an hour ago
    } else if (i > currentHour) {
      actual = undefined as any; // Future data not available
    } else {
      // Past hours closely match normal, with slight random variance
    ...
    if (actual !== undefined) {
      point.Actual = Math.round(actual);
    }
    data.push(point);
  }
  return data;
...
const generateWhiteLandChartData = (baseValueStr: string) => {
  ...
  return [
    ...
    { year: '2026', Value: val * 1.15, Penalty: val * 0.08 } // Projected
  ];
};
      ...
      {isOpen && (
        <>
          <motion.div 
            ...
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }} 
            className="fixed inset-0 z-40 bg-[#051005]/80 backdrop-blur-sm pointer-events-auto"
            onClick={onClose}
          />
          <motion.div 
            ...
            className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[540px] bg-[#0a140a]/95 border pointer-events-auto shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col uppercase tracking-wider font-bold"
          ...
          >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-[9px] text-gray-400 font-bold tracking-widest">
                      {data.type === 'commute' ? 'DETECTED ANOMALY' : data.type === 'white_land' ? 'IDLE LAND AGENT' : 'ASSET EVALUATION'}
                    </div>
             ...
             <div className="p-5 flex flex-col gap-4">
                  ...
                  <>
                    <div className="grid grid-cols-3 gap-4">
                      <div className="bg-[#051105]/80 border border-[#00B558]/20 p-3 rounded-sm flex flex-col justify-center">
                         <div className="text-[9px] text-[#00B558] mb-1 font-bold">NORMAL BASELINE</div>
                      ...
                      <div className="bg-[#051105]/80 border p-3 rounded-sm relative overflow-hidden flex flex-col justify-center" style={{ borderColor: `${color}40` }}>
                         ...
                         <div className="text-[9px] mb-1 relative z-10 font-bold" style={{ color }}>CURRENT STATE</div>
                         <div className="text-[20px] text-gray-100 font-black relative z-10">{data.currentSpeed} km/h</div>
                         <motion.div 
                      ...
                      <div className="bg-[#051105]/80 border border-gray-800 p-3 rounded-sm flex flex-col justify-center">
                         <div className="text-[9px] text-gray-400 mb-1 font-bold">ESTIMATED DELAY</div>
                    ...
                    <div className="bg-[#051105]/50 border border-gray-800 p-3 rounded-sm">
                      <div className="text-[9px] text-gray-400 mb-3 font-bold flex justify-between">
                        <span>24-HOUR TRAFFIC VELOCITY TREND</span>
                        <span style={{ color }}>CURRENT TIME: 17:00</span>
                      </div>
                          ...
                          <ComposedChart data={generateCommuteChartData(data.baselineSpeed, data.currentSpeed)} margin={{ top: 15, right: 10, left: -20, bottom: 0 }}>
                            <defs>
                              <linearGradient id="colorNormal" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="5%" stopColor="#00B558" stopOpacity={0.15}/>
                            ...
                            <Tooltip 
                              contentStyle={{ backgroundColor: '#051105', borderColor: '#374151', fontSize: '10px' }}
                ...
                {data.type === 'white_land' && (
                  <>
                    <div className="grid grid-cols-3 gap-4">
                      <div className="bg-[#051105]/80 border border-gray-800 p-3 rounded-sm flex flex-col justify-center">
                         <div className="text-[9px] text-gray-400 mb-1 font-bold">PARCEL SIZE</div>
                         <div className="text-[20px] text-gray-200 font-black">{data.size}</div>
                      </div>
                      <div className="bg-[#051105]/80 border border-gray-800 p-3 rounded-sm flex flex-col justify-center">
                         <div className="text-[9px] text-gray-400 mb-1 font-bold">EST. VALUE</div>
                         <div className="text-[20px] text-gray-200 font-black">{data.estValue}</div>
                      </div>
                      <div className="bg-[#051105]/80 border p-3 rounded-sm relative overflow-hidden flex flex-col justify-center" style={{ borderColor: `${color}40` }}>
                         <div className="absolute inset-0 opacity-10" style={{ backgroundColor: color }}></div>
                         <div className="text-[9px] mb-1 relative z-10 font-bold" style={{ color }}>TAX PENALTY</div>
                    ...
                    <div className="bg-[#051105]/50 border border-gray-800 p-3 rounded-sm">
                      <div className="text-[9px] text-gray-400 mb-3 font-bold flex justify-between">
                        <span>LAND VALUE VS TAX PENALTY ACCRUAL</span>
                        <span style={{ color }}>IDLE: {data.duration}</span>
                      </div>
                      <div className="h-[120px] w-full mt-2">
                        <ResponsiveContainer width="100%" height="100%">
                ...
                {data.type === 'roi' && (
                  <>
                    <div className="grid grid-cols-3 gap-4">
                      <div className="bg-[#051105]/80 border border-gray-800 p-3 rounded-sm flex flex-col justify-center">
                         <div className="text-[9px] text-gray-400 mb-1 font-bold">CURRENT YIELD</div>
                      ...
                      <div className="bg-[#051105]/80 border border-gray-800 p-3 rounded-sm flex flex-col justify-center relative">
                         <div className="text-[9px] text-gray-400 mb-1 font-bold">PROJECTED YIELD</div>
                         <div className="text-[20px] text-gray-200 font-black">{data.projectedYield}</div>
                      </div>
                      <div className="bg-[#051105]/80 border p-3 rounded-sm relative overflow-hidden flex flex-col justify-center" style={{ borderColor: `${color}40` }}>
                         <div className="absolute inset-0 opacity-10" style={{ backgroundColor: color }}></div>
                         <div className="text-[9px] mb-1 relative z-10 font-bold" style={{ color }}>10Y TREND</div>
                         <div className="text-[20px] font-black relative z-10 flex items-center gap-2" style={{ color }}>
                           {data.trend}
                         </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#051105]/50 border border-gray-800 p-3 rounded-sm">
                      <div className="text-[9px] text-gray-400 mb-3 font-bold flex justify-between">
                        <span>ROI FORECAST TRAJECTORY</span>
                        <span style={{ color }}>{data.trend === 'POSITIVE' ? 'APPRECIATION' : 'DEPRECIATION'}</span>
                      </div>
                      <div className="h-[120px] w-full mt-2">
                        <ResponsiveContainer width="100%" height="100%">
                          <ComposedChart data={generateROIChartData(data.currentYield, data.projectedYield)} margin={{ top: 15, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <ReferenceLine x="2024" stroke="#9CA3AF" strokeDasharray="3 3" opacity={0.5} label={{ position: 'insideTopLeft', value: 'NOW', fill: '#9CA3AF', fontSize: 9 }} />
                          </ComposedChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                ...
                <div className="bg-[#051105]/50 border border-gray-800 p-3 rounded-sm">
                  <div className="text-[10px] text-gray-400 mb-2 flex items-center gap-1.5 font-bold">
                    <Crosshair className="w-3.5 h-3.5" /> EVENT/ENVIRONMENTAL CAUSE
                  </div>
                ...
                <div className="mt-1 flex items-start gap-3 p-3 bg-[#051105]/80 border border-[#00B558]/30 rounded-sm relative overflow-hidden group">
                   ...
                   <div className="relative z-10">
                     <div className="text-[10px] text-[#00B558] mb-1.5 font-black tracking-widest">AI RECOMMENDATION</div>
                     <div className="text-[12px] text-gray-200 leading-relaxed normal-case font-medium">
                        {data.recommendation}
                     </div>
                   </div>
                </div>
             ...
             <div className="p-4 border-t border-gray-800 bg-[#051105]/90 flex justify-between items-center mt-auto">
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[11px] tracking-widest font-black" style={{ color }}>SEVERITY: {data.severity}</span>
                </div>
                <button 
                  onClick={onClose}
